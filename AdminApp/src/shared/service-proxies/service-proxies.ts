...
export class ProductServiceProxy {
     ...
     * @return Success
     */
    getAllAdminProduct(userId: number | undefined, body: SPInput | undefined): Observable<ServiceResponse> {
        let url_ = this.baseUrl + "/api/product/getAllAdminProduct?";
        if (userId === null)
            throw new Error("The parameter 'userId' cannot be null.");
        else if (userId !== undefined)
            url_ += "UserId=" + encodeURIComponent("" + userId) + "&";
        ...
        return this.http.request("post", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processGetAllAdminProduct(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetAllAdminProduct(<any>response_);
                } catch (e) {
                    return <Observable<ServiceResponse>><any>_observableThrow(e);
                }
            } else
    ...
    protected processGetAllAdminProduct(response: HttpResponseBase): Observable<ServiceResponse> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
